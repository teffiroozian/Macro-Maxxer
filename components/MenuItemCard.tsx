...
export default function MenuItemCard({
  ...
  const [selectedComboSideId, setSelectedComboSideId] = useState<string | undefined>(() => {
    ...
    return matchedSide ? (matchedSide.id ?? matchedSide.name) : comboConfig?.defaultSideId;
  });
  const [selectedComboDrinkId, setSelectedComboDrinkId] = useState<string | undefined>(() => {
    ...
    return matchedDrink ? (matchedDrink.id ?? matchedDrink.name) : comboConfig?.defaultDrinkId;
  });
  ...
  const customizationTotals = useMemo(
    () => ({
      calories: addonTotals.calories + ingredientCountTotals.calories + comboNutritionTotals.calories,
      protein: addonTotals.protein + ingredientCountTotals.protein + comboNutritionTotals.protein,
      carbs: addonTotals.carbs + ingredientCountTotals.carbs + comboNutritionTotals.carbs,
      totalFat: addonTotals.totalFat + ingredientCountTotals.totalFat + comboNutritionTotals.totalFat,
    }),
          ...
          {useCartQuickEditPanel ? (
              ...
              <div className="space-y-3">
                ...
                {comboType === "combo-meal" && selectedComboSide ? (
                  <section>
                    ...
                    <div className="grid grid-cols-[72px_minmax(0,1fr)] items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                      <div className="h-[72px] w-[72px] overflow-hidden rounded-lg border border-black/10 bg-white">
                        {selectedComboSideVariant?.image ?? selectedComboSide.image ? (
                          <img src={selectedComboSideVariant?.image ?? selectedComboSide.image} alt={selectedComboSide.name} className="h-full w-full object-contain p-1" />
                        ) : null}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-start justify-between gap-2">
                ...
                {comboType === "combo-meal" && selectedComboDrink ? (
                  <section>
                    ...
                    <div className="grid grid-cols-[72px_minmax(0,1fr)] items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                      <div className="h-[72px] w-[72px] overflow-hidden rounded-lg border border-black/10 bg-white">
                        {selectedComboDrinkVariant?.image ?? selectedComboDrink.image ? (
                          <img src={selectedComboDrinkVariant?.image ?? selectedComboDrink.image} alt={selectedComboDrink.name} className="h-full w-full object-contain p-1" />
                        ) : null}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-start justify-between gap-2">
                ...
                addons?.sauces?.items.some((addon) => (selectedSauceCounts[addon.name] ?? 0) > 0) ? (
                  <section>
                    <SectionEyebrow className="mb-2 px-1 text-[11px] text-slate-500">{addons?.sauces?.label ?? "Sauces"}</SectionEyebrow>
            ...
            <ItemDetailsPanel
              ...
              onSelectComboSide={(sideId) => {
                ...
                emitCartConfiguration(selectedVariantId, selectedAddons, selectedSauceCounts, ingredientCounts, comboType, sideId, selectedComboDrinkId, nextSideVariantId, selectedComboDrinkVariantId);
              }}

...
export default function ChipotleRestaurantBuilderView({
  ...
  const ingredientItemsById = useMemo(
    () =>
      new Map(
        ingredientMenuItems
          .filter((ingredient): ingredient is MenuItem & { id: string } =>
      ...
      Object.entries(selectedIngredientItems).reduce(
        (acc, [ingredientId, selectedIngredient]) => {
          ...
          return {
            calories:
              acc.calories +
              (nutrition.calories ?? 0) * selectedIngredient.quantity,
            protein:
              acc.protein +
              (nutrition.protein ?? 0) * selectedIngredient.quantity,
            carbs:
              acc.carbs + (nutrition.carbs ?? 0) * selectedIngredient.quantity,
            totalFat:
              acc.totalFat +
              (nutrition.totalFat ?? 0) * selectedIngredient.quantity,
          };
        },
      ...
      Object.entries(selectedIngredientItems).reduce(
        (acc, [ingredientId, selectedIngredient]) => {
          ...
          return {
            ...
            cholesterol:
              acc.cholesterol + (nutrition.cholesterol ?? 0) * quantity,
  ...
  const selectedBuildName = useMemo(() => {
    if (selectedEntree === "kids-meal") {
      const kidsMealLabel =
        selectedKidsMeal === "quesadilla" ? "Quesadilla" : "Build Your Own";
      const proteinLabel =
  ...
  const handleIngredientSelectionChange = (
    ...
    setSelectedIngredientItems((prev) => {
      ...
      if (category && typeof categoryMaxSelections === "number") {
        const selectedQuantityInCategory = getSelectedQuantityForCategory(
          prev,
          category,
        );
    ...
    (configuration: BuildConfigurationSnapshot) => {
      ...
      Object.entries(configuration.selectedIngredientItems).forEach(
        ([ingredientId, { quantity }]) => {
          ...
          const selectedVariantId =
            configuration.selectedIngredientVariantIds[ingredientId] ??
  ...
  const selectedIngredientEntries = useMemo(() => {
    ...
    const ingredientIndexById = new Map(
      ingredientMenuItems.map(
        (ingredient, index) => [ingredient.id ?? `${index}`, index] as const,
      ),
    );
    ...
    return [...selectedEntries].sort(
      ([leftId, leftIngredient], [rightId, rightIngredient]) => {
        const leftCategory = selectedIncludedIngredientIdSet.has(leftId)
          ? "included ingredient"
          : normalizeIngredientCategory(
              resolvePrimaryCategory(leftIngredient.item.categories),
            );
        const rightCategory = selectedIncludedIngredientIdSet.has(rightId)
          ? "included ingredient"
          : normalizeIngredientCategory(
              resolvePrimaryCategory(rightIngredient.item.categories),
            );
        const leftCategoryPriority =
          categoryPriority.get(leftCategory) ?? Number.POSITIVE_INFINITY;
        const rightCategoryPriority =
          categoryPriority.get(rightCategory) ?? Number.POSITIVE_INFINITY;
        ...
        const rightIngredientIndex =
          ingredientIndexById.get(rightId) ?? Number.POSITIVE_INFINITY;
        ...
        const selectedSplitIds = Object.entries(selectedIngredientItems)
          .filter(
            ([, splitSelectedIngredient]) =>
              normalizeIngredientCategory(
                resolvePrimaryCategory(splitSelectedIngredient.item.categories),
              ) === category,
          )
          .map(([id]) => id);
            ...
            <div className="grid justify-items-center gap-8">
              <div className="grid justify-items-center gap-5">
                ...
                <MacroTotalsGrid
                  macros={{
                    calories: Math.round(
                      adjustedSelectedIngredientTotals.calories ?? 0,
                    ),
                    protein: Math.round(
                      adjustedSelectedIngredientTotals.protein ?? 0,
                    ),
                    carbs: Math.round(
                      adjustedSelectedIngredientTotals.carbs ?? 0,
                    ),
                    totalFat: Math.round(
                      adjustedSelectedIngredientTotals.totalFat ?? 0,
                    ),
                  }}
                  size="panel"
                ...
                <MenuSections
                  ...
                  ingredientPortionModeOptionsById={(() => {
                    ...
                    > = Object.fromEntries(
                      visibleMenuItems
                        .filter(
                          (item) => item.id && isProteinIngredientItem(item),
                        )
                        .map((item) => [
                          item.id as string,
                          [
                  ...
                  selectedIngredientPortionModeIdById={(() => {
                    ...
                    (["rice", "beans"] as const).forEach((splitCategory) => {
                      const selectedSplitIds = Object.entries(
                        selectedIngredientItems,
                      )
                        .filter(
                          ([, selectedIngredient]) =>
                  ...
                  onIngredientVariantChange={(item, variantId) => {
                    ...
                    setSelectedIngredientItems((prev) => {
                      ...
                      const nextVariantNutrition =
                        ...
                        )?.nutrition ?? selectedIngredient.item.nutrition;
                      ...
                      const selectedModeById: Record<string, string> =
                        Object.fromEntries(
                          visibleMenuItems
                            .filter(
                              (item) =>
                                item.id && isProteinIngredientItem(item),
                            )
                            .map((item) => [
                              item.id as string,
                              proteinPortionMode,

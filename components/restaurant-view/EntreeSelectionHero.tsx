...
type Props = {
  onSelectEntree: (entree: Exclude<EntreeSelection, null>) => void;
};

export default function EntreeSelectionHero({ onSelectEntree }: Props) {
  return (
    <section className="mx-auto flex w-full max-w-5xl flex-col items-center px-4 pb-12">
      ...
      <div className="mt-10 grid w-full max-w-5xl gap-4 sm:grid-cols-3">
        {(Object.entries(CHIPOTLE_ENTREE_CONFIGURATIONS) as [Exclude<EntreeSelection, null>, EntreeConfiguration][]).map(
          ([entreeKey, entree]) => (
            <button
              key={entreeKey}
              type="button"

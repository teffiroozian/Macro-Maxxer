...
export default function ChipotleRestaurantView({
    restaurantId,
    restaurantName,
    restaurantLogo,
    ...
    const kidsBuildYourOwnDoubleSideIds = useMemo(
        () =>
            new Set(
                chipotleBuilderConfig?.chipotle
    ...
    const quesadillaTripleCheeseVariantId = useMemo(
        () =>
            chipotleBuilderConfig?.chipotle?.specialVariantIds
                ?.quesadillaTripleCheese ?? "quesadilla-triple-cheese",
        [chipotleBuilderConfig],
    );
    ...
    const selectedEntreeConfig = selectedEntree
        ...
        : null;
    const selectedEntreeportionMultiplier =
        selectedEntreeConfig?.portionMultiplier ?? 1;
    const tacoServingMultiplier =
        selectedEntree === "tacos" ? selectedTacoCount : 1;
    const servingMultiplier =
        selectedEntree === "tacos"
            ? selectedEntreeportionMultiplier
            : tacoServingMultiplier * selectedEntreeportionMultiplier;
    const ingredientDisplayMultiplier = selectedEntreeportionMultiplier;
    const tacoBaseServingCount = 3;
    const tacoSharedIngredientMultiplier =
        selectedEntree === "tacos"
            ? selectedTacoCount / tacoBaseServingCount
            : 1;
    const getIngredientportionMultiplier = useCallback(
        (ingredientId?: string) => {
    ...
    const selectedProteinCountForPortioning = useMemo(
        () =>
            Object.values(selectedIngredientItems).filter(
                (selectedIngredient) =>
        ...
        (ingredientId: string, category: string) => {
            ...
            if (category === "rice" || category === "beans") {
                const selectedSplitIds =
                    selectedSplitIngredientIdsByCategory[category];
                if (selectedSplitIds.length >= 2) {
                    return 0.5;
                }
                const portionMode =
                    splitPortionModeById[ingredientId] ?? "normal";
    ...
    const ingredientItemsById = useMemo(
        () =>
            new Map(
                ingredientMenuItems
                    .filter(
                        (ingredient): ingredient is MenuItem & { id: string } =>
    ...
    useEffect(() => {
        ...
        const updateActiveCategoryOnScroll = () => {
            const activationOffset =
                getStickyOffset() + SECTION_HEADER_TOP_GAP + 1;
            ...
            Object.entries(selectedIngredientItems).reduce(
                (acc, [ingredientId, selectedIngredient]) => {
                    const baseIngredient =
                        ingredientItemsById.get(ingredientId) ??
                    ...
                    const nutrition =
                        selectedVariant?.nutrition ?? baseIngredient.nutrition;
    ...
    const adjustedSelectedIngredientTotals = useMemo(() => {
        const scaledCalories =
            selectedIngredientTotals.calories * servingMultiplier;
        ...
        return {
            ...
            protein: Math.round(
                selectedIngredientTotals.protein * servingMultiplier,
            ),
            carbs: Math.round(
                selectedIngredientTotals.carbs * servingMultiplier,
            ),
            totalFat: Math.round(
                selectedIngredientTotals.totalFat * servingMultiplier,
            ),
        };
    }, [
        selectedEntree,
        selectedIngredientTotals,
        selectedKidsMeal,
        servingMultiplier,
    ]);
        ...
        () =>
            Object.entries(selectedIngredientItems).reduce(
                (acc, [ingredientId, selectedIngredient]) => {
                    ...
                    return {
                        calories:
                            acc.calories + (nutrition.calories ?? 0) * quantity,
                        totalFat:
                            acc.totalFat + (nutrition.totalFat ?? 0) * quantity,
                        satFat: acc.satFat + (nutrition.satFat ?? 0) * quantity,
                        transFat:
                            acc.transFat + (nutrition.transFat ?? 0) * quantity,
                        cholesterol:
                            acc.cholesterol +
                            (nutrition.cholesterol ?? 0) * quantity,
                        ...
                        protein:
                            acc.protein + (nutrition.protein ?? 0) * quantity,
                    };
                },
                {
                    calories: 0,
    ...
    const adjustedNutritionLabelTotals = useMemo(() => {
        const scaledCalories =
            selectedNutritionLabelTotals.calories * servingMultiplier;
        const shouldRoundKidsQuesadillaCalories =
            selectedEntree === "kids-meal" && selectedKidsMeal === "quesadilla";
        ...
        return {
            calories: shouldRoundKidsQuesadillaCalories
                ...
                : Math.round(scaledCalories),
            totalFat: Math.round(
                selectedNutritionLabelTotals.totalFat * servingMultiplier,
            ),
            satFat: Math.round(
                selectedNutritionLabelTotals.satFat * servingMultiplier,
            ),
            transFat: Math.round(
                selectedNutritionLabelTotals.transFat * servingMultiplier,
            ),
            cholesterol: Math.round(
                selectedNutritionLabelTotals.cholesterol * servingMultiplier,
            ),
            sodium: Math.round(
                selectedNutritionLabelTotals.sodium * servingMultiplier,
            ),
            carbs: Math.round(
                selectedNutritionLabelTotals.carbs * servingMultiplier,
            ),
            fiber: Math.round(
                selectedNutritionLabelTotals.fiber * servingMultiplier,
            ),
            sugars: Math.round(
                selectedNutritionLabelTotals.sugars * servingMultiplier,
            ),
            protein: Math.round(
                selectedNutritionLabelTotals.protein * servingMultiplier,
            ),
        };
    }, [
        selectedEntree,
        selectedKidsMeal,
        selectedNutritionLabelTotals,
        servingMultiplier,
    ]);

    const selectedIngredientCount = Object.values(
        selectedIngredientItems,
    ).reduce((acc, selectedIngredient) => acc + selectedIngredient.quantity, 0);
    ...
    const selectedBuildEntreeLabel = useMemo(() => {
        ...
        return (
            entreeOptions[selectedEntree]?.label ?? titleCase(selectedEntree)
        );
    ...
    const selectedBuildName = useMemo(() => {
        ...
        selectedBuildEntreeLabel,
        selectedBuildProteinNames,
        selectedEntree,
        selectedKidsMeal,
    ]);
    const selectedBuildImageSrc = useMemo(() => {
        ...
        if (selectedEntree === "kids-meal") {
            return (
                kidsMealOptions.find((option) => option.id === selectedKidsMeal)
                    ?.image ??
    ...
    const applyProteinPortionNutrition = useCallback(
        (
            ...
            const perProteinMultiplier = getProteinMultiplier(
                mode,
                proteinIds.length,
            );
    ...
    const applyIngredientPortionNutrition = useCallback(
        (
            ...
            const withProtein = applyProteinPortionNutrition(
                itemsById,
                options?.proteinMode,
            );
            return applySplitPortionNutrition(
                withProtein,
                options?.splitModesById,
            );
        },
    ...
    const getSelectedQuantityForCategory = (
        ...
        Object.values(itemsById).reduce((total, selectedIngredient) => {
            ...
            return selectedCategory === category
                ? total + selectedIngredient.quantity
                : total;
        }, 0);
    ...
    const handleIngredientSelectionChange = (
        ...
        if (
            selectedEntree === "tacos" &&
            tacoShellIngredientIds.includes(itemId)
        ) {
            if (!selected) return;
            const nextTacoShell =
        ...
        const nextSplitPortionModes = (() => {
            ...
            if (!selected) {
                ...
                if (currentSelectedSplitIds.length === 2) {
                    const remainingSplitId = currentSelectedSplitIds.find(
                        (splitId) => splitId !== itemId,
                    );
                    if (remainingSplitId) {
            ...
            if (
                currentSelectedSplitIds.length === 1 &&
                !currentSelectedSplitIds.includes(itemId)
            ) {
        ...
        setSelectedIngredientItems((prev) => {
            ...
            if (category && typeof categoryMaxSelections === "number") {
                const selectedQuantityInCategory =
                    getSelectedQuantityForCategory(prev, category);
            ...
            const selectedVariantId =
                selectedIngredientVariantIds[itemId] ?? item.defaultVariantId;
        ...
        if (item.defaultVariantId) {
            setSelectedIngredientVariantIds((prev) =>
                prev[itemId]
                    ? prev
                    : { ...prev, [itemId]: item.defaultVariantId as string },
            );
        }
    };
    ...
    const applyIncludedIngredients = useCallback(
        (
            ...
            const previouslyAppliedIncludedIngredientIds = new Set(
                appliedIncludedIngredientIdsRef.current,
            );
            const nextIncludedIngredientIdSet = new Set(
            ...
            setSelectedIngredientItems((previous) => {
                ...
                previouslyAppliedIncludedIngredientIds.forEach(
                    (ingredientId) => {
                ...
                nextIncludedIngredientIds.forEach((includedIngredientId) => {
                    const includedIngredientItem =
                        ...
                        (() => {
                            const fallbackIngredient = ingredients.find(
                                (ingredient) =>
                                    ingredient.id === includedIngredientId,
                            );
                            ...
                            return {
                                ...
                                defaultOrder:
                                    fallbackIngredient.defaultOrder ?? 0,
                                nutrition: fallbackNutrition,
                                variants: fallbackIngredient.variants?.map(
                                    (variant) => ({
                                        ...variant,
                                        nutrition: scaleNutritionValues(
                                            variant.nutrition,
                                ...
                                defaultVariantId:
                                    fallbackIngredient.defaultVariantId,
                                hideVariantSelector:
                                    fallbackIngredient.hideVariantSelector,
                    ...
                    const defaultVariant =
                        includedIngredientItem.variants?.find(
                            (variant) =>
                                variant.id ===
                                includedIngredientItem.defaultVariantId,
                        );
                    next[includedIngredientId] = {
                        item: {
                            ...
                            nutrition: useTripleCheese
                                ? scaleNutritionValues(
                                      includedIngredientItem.nutrition,
                                      3,
                                  )
                                : (defaultVariant?.nutrition ??
                                  includedIngredientItem.nutrition),
                        },
                        quantity: 1,
                    };
    ...
    const buildSelectedItemsFromConfiguration = useCallback(
        (configuration: BuildConfigurationSnapshot) => {
            ...
            Object.entries(configuration.selectedIngredientItems).forEach(
                ([ingredientId, { quantity }]) => {
                    ...
                    const selectedVariantId =
                        configuration.selectedIngredientVariantIds[
                            ingredientId
                        ] ?? ingredient.defaultVariantId;
                    const selectedVariant = ingredient.variants?.find(
                        (variant) => variant.id === selectedVariantId,
                    );
    ...
    const handleAddBuildToCart = () => {
        ...
        const nextCustomizations = Object.entries(
            selectedIngredientItems,
        ).flatMap(([ingredientId, { item, quantity }]) => {
        ...
        const nextItemPayload = {
            ...
            name:
                editingCartItem?.name && selectedEntree !== "kids-meal"
    ...
    useEffect(() => {
        ...
        const configuration = fromUniversalChipotleBuildConfiguration(
        ...
        const { missingIngredientIds, selectedItems } =
            buildSelectedItemsFromConfiguration(configuration);
    ...
    const adjustIngredientQuantity = (ingredientId: string, delta: 1 | -1) => {
        ...
        setSelectedIngredientItems((previous) => {
            ...
            const category = normalizeIngredientCategory(
                resolvePrimaryCategory(existing.item.categories),
            );
            const categoryMaxSelections = getIngredientCategoryMaxSelections({
                category,
                selectedEntree,
                ...
                delta > 0 && typeof categoryMaxSelections === "number"
                    ? Math.max(
                          ...
                          categoryMaxSelections -
                              (selectedQuantityInCategory - existing.quantity),
                      )
                    : undefined;
    ...
    const handleResetSelectedIngredientOrder = () => {
        if (isEditingBuild && editingBuildBaselineConfigRef.current) {
            ...
            setSelectedIngredientVariantIds(
                baselineConfiguration.selectedIngredientVariantIds,
            );
            setSelectedIngredientItems(
                () =>
                    buildSelectedItemsFromConfiguration(baselineConfiguration)
                        .selectedItems,
            );
            return;
        }
    ...
    const selectedIngredientEntries = useMemo(() => {
        ...
        const categoryPriority = new Map<string, number>(
            (chipotleBuilderConfig?.selectedIngredientCategoryOrder ?? []).map(
                (category, index) => [category, index] as const,
            ),
        );
        const ingredientIndexById = new Map(
            ingredientMenuItems.map(
                (ingredient, index) =>
                    [ingredient.id ?? `${index}`, index] as const,
            ),
        );

        return [...selectedEntries].sort(
            ([leftId, leftIngredient], [rightId, rightIngredient]) => {
                const leftCategory = selectedIncludedIngredientIdSet.has(leftId)
                    ? "included ingredient"
                    : normalizeIngredientCategory(
                          resolvePrimaryCategory(
                              leftIngredient.item.categories,
                          ),
                      );
                const rightCategory = selectedIncludedIngredientIdSet.has(
                    rightId,
                )
                    ? "included ingredient"
                    : normalizeIngredientCategory(
                          resolvePrimaryCategory(
                              rightIngredient.item.categories,
                          ),
                      );
                ...
                const leftIngredientIndex =
                    ingredientIndexById.get(leftId) ?? Number.POSITIVE_INFINITY;
                const rightIngredientIndex =
                    ingredientIndexById.get(rightId) ??
                    Number.POSITIVE_INFINITY;
                ...
                return leftIngredient.item.name.localeCompare(
                    rightIngredient.item.name,
                );
            },
        );
    }, [
    ...
    const groupedSelectedIngredientEntries = useMemo(() => {
        ...
        selectedIngredientEntries.forEach((entry) => {
            ...
            const categoryKey = selectedIncludedIngredientIdSet.has(
                ingredientId,
            )
                ? "included ingredient"
                : normalizeIngredientCategory(
            ...
            const existingGroup = groupedEntries.find(
                (group) => group.categoryKey === categoryKey,
            );
            if (existingGroup) {
            ...
            groupedEntries.push({
                categoryKey,
                categoryLabel:
                    chipotleBuilderConfig?.selectedIngredientCategoryLabels?.[
                        categoryKey
                    ] ?? "Ingredient",
                entries: [entry],
            });
    ...
    const ingredientPortionLabelById = (() => {
        const labelById: Record<string, string> =
            ...
            selectedKidsMeal === "build-your-own"
                ? Object.fromEntries(
                      Array.from(kidsBuildYourOwnDoubleSideIds).map(
                          (ingredientId) => [ingredientId, "2x"],
                      ),
                  )
                : {};
                ...
                const selectedSplitIds = Object.entries(selectedIngredientItems)
                    .filter(
                        ([, splitSelectedIngredient]) =>
                            normalizeIngredientCategory(
                                resolvePrimaryCategory(
                                    splitSelectedIngredient.item.categories,
                                ),
                            ) === category,
                    )
                    .map(([id]) => id);
    ...
    const unavailableIngredientIds = useMemo(() => {
        ...
        const selectedCategoryQuantities = Object.values(
            selectedIngredientItems,
        ).reduce<Record<string, number>>((acc, selectedIngredient) => {
            const category = normalizeIngredientCategory(
                resolvePrimaryCategory(selectedIngredient.item.categories),
            );
    ...
    const unavailableIngredientReasonById = useMemo(
        () =>
            Object.fromEntries(
                Array.from(unavailableIngredientIds).map((ingredientId) => [
                    ingredientId,
                    "Category max reached",
                ]),
            ),
        ...
        return (
            ...
            >
                ...
                <div className="relative flex h-[calc(100vh-1rem)] w-full max-w-[1024px] flex-col overflow-hidden rounded-2xl bg-white px-3 pt-3 sm:h-[calc(100vh-2rem)] sm:px-5 sm:pt-5 lg:px-6 lg:pt-6">
                    ...
                    <div
                        ref={buildCustomizationModalScrollRef}
                        className="min-h-0 flex-1 overflow-y-auto pb-10 pr-1 sm:pr-2 [overflow-anchor:none]"
                    >
                        <div className="grid justify-items-center gap-8">
                            <div className="grid justify-items-center gap-5">
                                ...
                                <MacroTotalsGrid
                                    macros={{
                                        calories: Math.round(
                                            adjustedSelectedIngredientTotals.calories ??
                                                0,
                                        ),
                                        protein: Math.round(
                                            adjustedSelectedIngredientTotals.protein ??
                                                0,
                                        ),
                                        carbs: Math.round(
                                            adjustedSelectedIngredientTotals.carbs ??
                                                0,
                                        ),
                                        totalFat: Math.round(
                                            adjustedSelectedIngredientTotals.totalFat ??
                                                0,
                                        ),
                                    }}
                                    size="panel"
                            ...
                            <div className="w-full rounded-3xl border border-black/10 bg-[#e0e0e0] p-4">
                                <MenuSections
                                    ...
                                    ingredientPortionModeOptionsById={(() => {
                                        ...
                                        return Object.keys(optionsById).length >
                                            0
                                            ? optionsById
                                            : undefined;
                                    })()}
                            ...
                            <div className="w-full rounded-3xl border border-black/10 bg-[#e0e0e0] p-4">
                                <BuildSummaryDrawer
                                    ...
                                    onResetOrder={
                                        handleResetSelectedIngredientOrder
                                    }
                                    onSaveOrder={
                                        handleSaveSelectedIngredientOrder
                                    }
                    ...
                    <div className="-mx-3 z-10 flex flex-wrap items-center gap-3 border-t border-black/10 bg-white p-3 shadow-[0_-4px_10px_rgba(0,0,0,0.08)] sm:-mx-5 sm:p-4 lg:-mx-6">
                        <MacroTotalsGrid
                            macros={{
                                ...
                                carbs: Math.round(
                                    adjustedSelectedIngredientTotals.carbs ?? 0,
                                ),
                                totalFat: Math.round(
                                    adjustedSelectedIngredientTotals.totalFat ??
                                        0,
                ...
                >
                    <span className="relative h-5 w-5 shrink-0 overflow-hidden rounded-full border border-slate-200 bg-white">
                        <Image
                            ...
                            alt={
                                entreeOptions[selectedEntree]?.label ??
                                selectedEntree
                            }
                            fill
                ...
                {isEntreeMenuOpen ? (
                    ...
                    >
                        <div className="grid gap-1">
                            ...
                            {Object.entries(entreeOptions).map(
                                ([entreeKey, entree]) => {
            ...
            {shouldShowBuildStickyBar ? (
                <div ref={buildStickyContainerRef}>
                    <StickyMacroTotalsBar
                        ...
                        primaryActionLabel={
                            isEditingBuild ? "Save & Add" : "Add to Cart"
                        }

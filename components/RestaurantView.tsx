...
export default function RestaurantView({
  ...
  const closeBuildEditModal = useCallback(() => {
    const nextParams = new URLSearchParams(searchParams.toString());
    nextParams.delete("editCartItem");
    const nextQuery = nextParams.toString();
    router.replace(nextQuery ? `${pathname}?${nextQuery}` : pathname, { scroll: false });
            ...
            <div className="h-[calc(100%-52px-80px)] overflow-y-auto pb-6 pr-1">
              <div className="mx-auto grid w-full max-w-[900px] gap-6">
                <section className="grid justify-items-center gap-6">
                  ...
                  {selectedBuildImageSrc ? (
                    <img
                      src={selectedBuildImageSrc}
                      alt={selectedBuildName}
                      className="h-[300px] w-[300px] rounded-[14px] bg-[#efefef] object-contain shadow-[0_0_5px_rgba(0,0,0,0.25)]"
                    />
                  ) : null}
                  <MacroTotalsGrid
                ...
                <section className="rounded-[18px] bg-[#efefef] p-4">
                  <MenuSections
                    restaurantId={restaurantId}
                    items={visibleMenuItems}
                    sort={sort}

...
import {
  ...
  CHIPOTLE_PROTEIN_FAMILIES,
  CHIPOTLE_TOPPING_FAMILIES,
  CHIPOTLE_TORTILLA_FAMILIES,
  type ContextVariantFamilyMember,
} from "@/lib/restaurantBuilders/chipotle/canonicalIngredients";
...
// View All Ingredients only: several official Chipotle tortilla/topping
// records are the exact same product at different serving contexts,
// generated as separate ids because the per-entree builder needs to pick the
// right one for the current build. That distinction is meaningless for an
// entree-agnostic browse/compare card, so this presentation-only grouping
// folds each family's context records into one card exposed as selectable
// variants — the generated ids/nutrition underneath are untouched and still
// drive the actual builder via buildChipotleIngredientMenuItems. The
// underlying id relationships live in canonicalIngredients.ts, shared with
// the search index's dedup so both surfaces agree on what's a duplicate.
const chipotleAllIngredientsTortillaFamilyByPrimaryId = new Map(
  CHIPOTLE_TORTILLA_FAMILIES.map((family) => [family.primaryId, family]),
);
const chipotleAllIngredientsTortillaVariantMemberIds = new Set(
  CHIPOTLE_TORTILLA_FAMILIES.flatMap((family) =>
    family.variants
...
const CHIPOTLE_ALL_INGREDIENTS_HIDDEN_DUPLICATE_IDS = CHIPOTLE_HIDDEN_DUPLICATE_CANONICAL_ID_BY_ID;
...
const chipotleAllIngredientsToppingFamilyByPrimaryId = new Map(
  CHIPOTLE_TOPPING_FAMILIES.map((family) => [family.primaryId, family]),
);
const chipotleAllIngredientsToppingVariantMemberIds = new Set(
  CHIPOTLE_TOPPING_FAMILIES.flatMap((family) =>
    family.variants
      .map((variant) => variant.id)
      .filter((id) => id !== family.primaryId),
  ),
...
const chipotleAllIngredientsProteinPrimaryIds = new Set(
  CHIPOTLE_PROTEIN_FAMILIES.map((family) => family.primaryId),
);
const chipotleAllIngredientsProteinVariantMemberIds = new Set(
  CHIPOTLE_PROTEIN_FAMILIES.flatMap((family) => family.contextDuplicateIds),
);
...
const CHIPOTLE_ALL_INGREDIENTS_NON_PROTEIN_IDS = CHIPOTLE_NON_PROTEIN_CANONICAL_ID_BY_ID;
...
export function buildChipotleIngredientMenuItems({
  ...
  const mappedIngredientItems = ingredients
    ...
    .map((ingredient, index) => {
      ...
      const tortillaSideCategoryOverride =
        ingredientId === tortillaSideIngredientId && selectedEntree
          ? builderConfig?.chipotle?.tortillaSideCategoryByEntree?.[selectedEntree]
          : undefined;
      const displayCategory = shouldPinToIncludedCategory
        ? "Included Ingredients"
        : (tortillaSideCategoryOverride ?? resolvedCategory);

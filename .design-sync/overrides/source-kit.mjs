...
// forked from design-sync lib/source-kit.mjs - synth-entry `export *` misses
// default exports; Macro Maxxer's components/ui/*.tsx use `export default`.
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Files whose PascalCase basename is excluded via cfg.componentSrcMap:
    // {Name: null} are dropped from the synth entry entirely, not just from
    // the component list - a bundled-but-excluded file (e.g. one importing
    // next/image, whose module-level code touches process.env.__NEXT_*
    // outside the Next.js runtime) still runs its top-level side effects in
    // the shared IIFE and can break every other component's evaluation.
    const comps = srcFiles
      .filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p))
      .filter((p) => srcMap[basename(p, extname(p))] !== null);
    entry = join(OUT, '.pkg-entry.mjs');
    writeFileSync(
      entry,
      comps
        .map((p) => {
          const named = `export * from ${JSON.stringify(p)};`;
          const inferred = basename(p, extname(p));
          // Re-export the default under its filename too - components/ui/*.tsx
          // ship `export default X`, which `export *` never surfaces.
          return /^[A-Z]/.test(inferred)
            ? `${named}\nexport { default as ${inferred} } from ${JSON.stringify(p)};`
            : named;
        })
        .join('\n') + '\n',
    );
